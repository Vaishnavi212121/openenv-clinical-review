...
class ClinicalTrialReviewEnv:
    ...
    def step(self, action: Action | Dict[str, Any]) -> Tuple[Observation, Reward, bool, Dict[str, Any]]:
        ...
        reward = Reward(
            value=round(max(-1.0, min(1.0, step_reward_value)), 4),
        ...
        if done:
            task_result = self._compute_final_score()
            info["task_result"] = task_result.model_dump()
            obs.info["task_result"] = task_result.model_dump()

        return obs, reward, done, info

    def state(self) -> EpisodeState:
        """Return full internal state (for debugging, not for agents in eval)."""
        if self._state is None:
    ...
    def _action_read_section(self, action: Action) -> Dict[str, Any]:
        ...
        self._state.current_section = section_name
        reward = 0.0
        components = {}

        # Reward for reading a new section
        if section_name not in self._state.sections_reviewed:
            ...
            if has_issues:
                reward += 0.04  # Small reward for navigating to a section with issues
                components["new_section_with_issues"] = 0.04
            else:
                reward += 0.01  # Very small reward for thoroughness
                components["new_section_no_issues"] = 0.01
        else:
            reward += -0.01  # Small penalty for re-reading (inefficient)
            components["repeated_read"] = -0.01

        return {
            "reward": reward,
            "components": components,
            "info": {"section_loaded": section_name, 
                    "has_issues": sections[section_name].get("has_issues", False),
    ...
    def _action_raise_flag(self, action: Action) -> Dict[str, Any]:
        ...
        # Immediate intermediate reward based on flag quality
    ...
    def _action_submit(self) -> Dict[str, Any]:
        ...
        terminal_reward = result.score * 0.5  # Final score contributes to terminal reward
        return {
    ...
    def _action_clarification(self, action: Action) -> Dict[str, Any]:
        ...
        # Clarification reveals whether current section has issues (costs a step)
        ...
        return {
            "reward": -0.02,  # Small penalty for using clarification
    ...
    def _score_flag_intermediate(self, flag: ProtocolFlag) -> Tuple[float, Dict[str, float]]:
        ...
        This is the key dense reward mechanism.
        """
        gt_issues = self._state.ground_truth_issues
        components = {}

        # Check if flag matches any GT issue
        ...
        if match_id:
            ...
            # Check if it was already found (diminishing returns)
            already_found = sum(
                1 for f in self._state.flags_raised[:-1]
                if _match_flag_to_gt(f, [gt_issue])[0] is not None
            )
            if already_found > 0:
                partial_reward *= 0.1  # Strongly diminishing returns for duplicate
    ...
    def _handle_invalid_action(self, action: Action) -> Tuple[Observation, Reward, bool, Dict[str, Any]]:
        ...
        reward = Reward(value=penalty, cumulative=self._state.cumulative_reward,
                       components={"invalid_action": penalty}, reason="Invalid action format")
